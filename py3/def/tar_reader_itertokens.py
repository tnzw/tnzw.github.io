# tar_reader_itertokens.py Version 1.0.0
  ...
  while 1:
    ...
    if not any(block):
      ...
      if not any(next_block):
        ...
        for _ in io_iterread1(reader, length=RECORD_SIZE - (tar_position % RECORD_SIZE), size=buffer_size):
    ...
    if data_size:
      ...
      for _ in io_iterread1(reader, length=data_size, size=buffer_size):
        yield "data_chunk", _
      yield "data_chunk_end", b""
      padding_size = data_size % 512
      if padding_size: padding_size = 512 - padding_size
      for _ in io_iterread1(reader, length=padding_size, size=buffer_size):
