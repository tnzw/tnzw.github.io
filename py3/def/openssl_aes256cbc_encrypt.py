# openssl_aes256cbc_encrypt.py Version 1.1.0
...
def openssl_aes256cbc_encrypt():
  ...
  def openssl_aes256cbc_encrypt(message, password, salt=None, iv=None):
    ...
    res = []
    for _ in _iter(message, password, salt, iv): res.append(_)
    return b"".join(res)

  def _iter(message, password, salt=None, iv=None):
    ...
    message = aes_cbc_pkcs7_encryptbytes.iter(message, transcode_uint8_to_uint32_bigendian(key), transcode_uint8_to_uint32_bigendian(iv if iv else generated_iv))
    if salt: yield b"Salted__" + bytes(salt)
    yield from message

  openssl_aes256cbc_encrypt.iter = _iter
  return openssl_aes256cbc_encrypt

openssl_aes256cbc_encrypt = openssl_aes256cbc_encrypt()
