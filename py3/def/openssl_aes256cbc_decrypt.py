# openssl_aes256cbc_decrypt.py Version 1.0.1
...
def openssl_aes256cbc_decrypt(message, password, with_salt=None, iv=None):
  ...
  if with_salt in (None, True):
    if L is None:
      ...
      if len(extract) == 16 and extract[:8] == b"Salted__":
        ...
        if with_salt: return None  # salt desired but not found
      ...
      if len(extract) == 16 and extract[:8] == b"Salted__":
        salt = extract[8:16]
        message = message[16:]
      elif with_salt: return None  # salt desired but not found
  ...
  return aes_cbc_pkcs7_decryptbytes(message, transcode_uint8_to_uint32_bigendian(key), transcode_uint8_to_uint32_bigendian(iv if iv else generated_iv))
