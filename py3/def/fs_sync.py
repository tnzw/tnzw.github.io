...
"""
    ...
    def sync(       srcname, dstname, src_dir,         dst_dir,         src_lstats=None,       dst_lstats=None,       src_stats=None,      dst_stats=None,      _in_delete=False,      _in_backup=False,      follow_symlinks=False,           custom_check=False,        _src_dirlist=None,         _dst_dirlist=None,         source_directory=False,            target_directory=False,            src_noent_ok=True,         content=False,   head=None, recursive=False,     backup=False,  backup_dir=False,      suffix=None,   inplace=False,   temp_dir=None,     update=False,  dirs=False, links=False, copy_links=False,      copy_dirlinks=False,         keep_dirlinks=False,         perms=False, executability=False,         chmod=None,  owner=False, group=False, devices=False,   specials=False,    times=False, omit_dir_times=False,          omit_link_times=False,           _omit_link_copystat=True,                dry_run=False,   ignore_non_existing=False,               ignore_existing=False,           remove_source_files=False,               remove_source_dirs=False,              delete=False,  force=False, chown=False, chown_uid=-1,        chown_gid=-1,        ignore_times=False,        size_only=False,     times_only=False,      modify_window=None,          src_time_offset=None,            exclude=lambda _: False, include=lambda _: False, file_matcher=None,         copy_function=None,          yield_nodes=None,        yield_sync=None,       yield_skip=None,       yield_os_calls=None,           yield_sync_errors=None,              yield_listdir_errors=None,                 _top_verbose=True,         verbose=0,       onverbose=None,      ignore_errors=False,         ignore_sync_errors=None,               ignore_listdir_errors=False,                 ignore_exdev_errors=True,                onerror=None,    buffer_size=None,        _open=open,  _open_kw={},       os_module=None,      _os=None, sep=None, altsep=None):
      ...
      def same(*a, **k):
        if 0: yield  # XXX enable yields because fs_diff is not yielding
        os_modules = None if os_module is None else ((os_module,) * len(a))
        return fs_diff(*a, os_modules=os_modules, **k)  # XXX make it yielding
      ...
      def diffmodifywindow(i, j):
        ...
        if modify_window is None: return int(i) != int(j)  # XXX using int() because sometimes it compares 1589402770.3552 with 1589402770.3575144. Should we use int() before this line ? like i,j=int(i),int(j)
        if modify_window == 0: return i != j
      ...
      def difffile():
        if custom_check:
          ...
          if content:
            equals = yield from same(src, dst, compare_size=False, max_length=None if head is None or head < 0 else head, stats=(src_stats, dst_stats))
            if not equals: return 1
          elif head is not None:
            equals = yield from same(src, dst, max_length=None if head < 0 else head, stats=(src_stats, dst_stats))
            if not equals: return 1
          return 0
      ...
      def opentmp():
        ...
        if 0: yield  # XXX enable generator because opennewfile is not yielding
        tmpf = opennewfile.temp(_, 'xb', base_format='.{base}.{rand}', os_module=os_module)  # XXX make it yielding
        tmp = tmpf.name
      def closetmp():
        if 0: yield  # enable generator
        if dry_run: return
      ...
      if copy_function:
        def copyfileinplace(src, dst):
          ...
          if 0: yield  # XXX enable generator because copy_function is not yielding
          if dry_run: return
        ...
        def copyfileinplace(src, dst):
          ...
          if 0: yield  # XXX enable generator because fs_copyfile is not yielding
          if dry_run: return
          try: dst = dst if isinstance(dst, int) else dst.fileno()
          except AttributeError: pass
          return fs_copyfile(src, dst, buffer_size=buffer_size, os_module=os_module)  # XXX make it yielding so that we can yield_copy_progess

      def copyfileintmp(src):
        yield from copyfileinplace(src, tmpf)
        ...
        for name in g:
          ...
          try: yield from _sync(*args)
            ...
            if ignore_sync_errors: pass
              ...
              if yield_sync_errors and _yerr((yield ('sync_error', _sync, args, sys.exc_info()))): pass
      ...
      try:
        ...
        if tmpf is not None:
          tmpf.close()
          try: yield from unlink(tmpf.name)  # prevent internal yields to stop the unlink process? I think no.
          except OSError: pass
        elif tmp is not None:
          try: yield from unlink(tmp)  # prevent internal yields to stop the unlink process? I think no.
          except OSError: pass
      # End def sync()
