this.createCodePointArrayFromUtf8CodeArrayLikeFirefox = (function script() {
  "use strict";

  /*! createCodePointArrayFromUtf8CodeArrayLikeFirefox.js Version 1.0.0
  ...
  function createCodePointArrayFromUtf8CodeArrayLikeFirefox(bytes, i, l) {
    ...
    while (i < l) {
      ...
      if (error) {
        ...
        if (error === "invalid continuation byte") {
	      ...
	      if (bytes[i - 1] === 0xF4) {
                if (bytes[i] < 0x90) i += 1;
	      } else if (bytes[i - 1] >= 0xF1) {
                i += 1;
              } else {
                if (bytes[i] < 0x90) {
                  if (bytes[i + 1] >= 0x90 && bytes[i + 1] < 0xC0) i += 1;
                } else i += 1;
              }
            } else if (length === 4) {
        ...
        } else if (error === "unexpected end of data") {
          if (result.expectedLength === 3) {
            if (length === 2) {
              // here bytes[i] is already between 0x80 <= and < 0xC0
              if ((bytes[i - 1] !== 0xE0 || bytes[i] >= 0xA0) &&
                  (bytes[i - 1] !== 0xED || bytes[i] < 0xA0)) i += 1;
	    }
          } else if (result.expectedLength === 4) {
            if (length === 2) {
              // here bytes[i] is already between 0x80 <= and < 0xC0
              if ((bytes[i - 1] !== 0xF0 || bytes[i] >= 0x90) &&
                  (bytes[i - 1] !== 0xF4 || bytes[i] < 0x90)) i += 1;
	    } else if (length === 3) {
              // here bytes[i] and [i + 1] are already between 0x80 <= and < 0xC0
              if (bytes[i - 1] === 0xF4) {
                if (bytes[i] < 0x90) i += 2;
              } else if (bytes[i - 1] !== 0xF0 || bytes[i] >= 0x90) i += 2;
	    }
          }
        }
      } else {
  ...
  createCodePointArrayFromUtf8CodeArrayLikeFirefox.toScript = function () { return "(" + script.toString() + "())"; };
  createCodePointArrayFromUtf8CodeArrayLikeFirefox._requiredGlobals = ["decodeCodePointFromUtf8CodeArray"];
  return createCodePointArrayFromUtf8CodeArrayLikeFirefox;
